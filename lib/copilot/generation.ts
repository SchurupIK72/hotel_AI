...
export async function resolveLatestInboundMessageId(
  supabase: CopilotSupabaseClient,
  hotelId: string,
  conversationId: string,
) {
  return getLatestInboundMessageId(supabase, hotelId, conversationId);
}
...
export async function regenerateConversationDrafts(input: {
  hotelId: string;
  conversationId: string;
}) {
  const { createServerSupabaseClient } = await import("../supabase/server.ts");
  ...
  if (!latestInboundMessageId) {
    throw new Error("No inbound guest message is available for draft generation yet.");
  }

  return generateConversationDraftsWithClient(supabase, {
    hotelId: input.hotelId,
    conversationId: input.conversationId,
    triggerMessageId: latestInboundMessageId,
